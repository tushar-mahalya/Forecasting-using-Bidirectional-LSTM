...
def predictor_response_split(df: pd.DataFrame, window_size: int, multi_seq: bool = False):
    X = []
    y = []
    if multi_seq:
...
def train_test_data(df: pd.DataFrame, split_size: float = 0.2, window_size: int = 7, multi_seq: bool = False):
    X, y = predictor_response_split(df, window_size, multi_seq)
